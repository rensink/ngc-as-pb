...
Now consider $\cF=\rF v x$ (where $x;v=\id_B$). Let $s=\spanof u d$, $\rF v x=s_1=\spanof{u_1}{d_1}$ and $\dF v(s)=s_2=\spanof{u_2}{d_2}=\spanof{u;v}{d}$. Now $k=x'$ as in \fcite{sb-source shifters} shows $(s_1,s_2)\in \cP_\id$ according to \eqcite{pattern shift}.

\item First consider $\cB=\rB v x$ where $x;v=\id_B$. Referring to \fcite{sb-source shifters}, it is immediately clear that $\dF v(\cB(s))=s$ for all $s$; since $\ccP_\id$ includes the identity over $B$-sourced spance, we are done.
