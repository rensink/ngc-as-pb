...
\begin{definition}[{\fprovable} entailment/equivalence]\label{def:provable}
...
%
\begin{proof}
Each $b_i\pentails c_i$ follows from the transitive closure of a sequence of primitive steps (i.e., one of the two cases of Def.~\ref{def:provable}). It therefore suffices to prove that, if \emph{a single} $p_i\pentails c_i$ is due to the existence of either\todo{AC: delete `backward' everywhere if not needed. AR: Done}
\begin{inumerate}
...
\begin{itemizeS}
\item $\cB=\rB v x$ where $x;v=\id_B$. From \lcite{sb-shifter inverses} we know that $\dF v(\cB(s))=s$ for all $s$; since $\ccP_\id$ includes the identity over $\cat{C^{sp}}(B,\_)$, we are done.
