...
\node (E) [right=of R2] {$E$};
\path (E) edge [left hook->] node[above] {$e$} (R2);
\end{tikzpicture}
\end{equation}
%
The diagram is called an \emph{arrow-based reflection square} if, for the pushout $g,h$ of the span $v,a_2$ as drawn in the middle diagram, the bottom left subdiagram commutes, i.e., $g=a_1;v';h$. It is called a \emph{simple arrow-based reflection square} if, for the equaliser $s$ of $a_2$ and $v;a_1;v'$ as drawn in the right hand diagram, $e;v$ is epi.
\end{definition}
%
...
\item\label{simple reflection square any}  In the left hand diagram of \eqref{eq:reflection square}, if there is \emph{any} $e'\of E'\to R_2$ (not necessarily the equaliser) such that $e';a_2=e';v;a_1;v'$ and $e';v$ is epi, then the diagram is a simple reflection square.

\begin{equation}\label{eq:simple reflection square any}
\begin{tikzpicture}[on grid,auto,node distance = 1.5cm,baseline=(a1)]
\node (R1) {$R_1$};
\node (P1) [below=of R1] {$P_1$};
\node (R2) [right=of R1] {$R_2$};
...
\begin{proof}[]
\begin{enumerate}[defsep]
\item The equaliser guarantees $e;a_2;h=e;v;a_1;v';h$, and the pushout guarantees $e;a_2;h=s;v;g$; hence $e;v;a_1;v';h=e;v;g$. Since $e;v$ is epi, it follows that $a_1;v';h=g$.

\item Since the diagram already commutes, $e=\id$ is the equaliser; hence $e;v=v$ is epi.

\item Let $e'$ be as in \eqref{eq:simple reflection square any} and let $e$ be the equaliser of $a_2$ and $v;a_1;v'$ as on the right hand side of \eqref{eq:reflection square}. Due to the universal property of the equaliser, there is a (unique) $k\of E'\to E$ such that $e'=k;e$. But then, since $e'$ is epi, so is $e$.

\item The section $s$ in \eqref{eq:reflection square section} satisfies the conditions on $e'$ in Clause~\ref{simple reflection square any} (noting that $s;v=\id$ is epi).

\item Consider the following diagram, where $e_1$ is the equaliser of $a_2$ and $v_1;a_1;v_1'$, $e_2$ is the equaliser of $a_3$ and $v_2;a_2;v_2'$, and $e_1',e_2'$ form the pullback of $e_1$ and $e_2;v_2$.
%
\[
\begin{tikzpicture}[on grid,auto,node distance = 1.5cm,baseline=(a1)]
\node (R1) {$R_1$};
...
\]
%
Note that \tagone $e_1;a_2=e_1;v_1;a_2;v_1'$ and \tagtwo $e_2;a_3=e_2;v_2;a_2;v_2'$. Using also that \tagthree the pullback square commutes, we can deduce
\begin{align*}
...
\end{align*}
%
Moreover, both $e_1;v_1$ and $e_2;v_2$ are epi. Epis are preserved by pullback (in presheaf toposes), hence $e_2'$ is epi. Finally, epis compose, hence $e_1';e_2;v_2;v_1=e_2';e_1;v_1$ is epi. Thus, $e_1';e_2$ satisfies the conditions of Clause~\ref{simple reflection square any}.
...
\item\label{preservation square any} Given a preservation square, for any pair of arrows $g,h$ such that $g=a_1;h$, there is an arrow $h'$ such that $h=v';h'$ and $v;g=a_2;h'$.
...
\begin{proof}[]
\begin{enumerate}[defsep]
\item If $v'$ is iso in the left hand diagram of \eqref{eq:preservation square}, then it is split mono with retract $r'=v'^{-1}$. If the diagram commutes, than $a_2=v;a_1;v'$ and hence $a_2;r'=v_1;a_1$, proving the preservation square property. Vice versa, $a_2;r'=v_1;a_1$ implies $a_2=a_2;r';v'=v_1;a_2;v'$, proving commutativity of the original square.

\item Define $h'=r';h$; then $v';h'=v';r';h=h$ and $v;g=v;a_1;h=a_2;r';h=a_2;h'$ as required.
...
\begin{definition}[arrow-based condition morphism]\label{def:ab-pre-morphism}
\begin{itemize}[defsep]
\item Let $c_i = (R_i,p^i_1\ccdots p^i_{w_i})\in \AC{R_i}$ for $i=1,2$. An \emph{ab-condition reflection [preservation] morphism} (or just ab-reflection [-preservation] pre-morphism) $m\of c_1 \to c_2$ is a tuple $(v,o,m_1\ccdots m_{w_2})$ such that
\begin{itemize}[nosep]
...
\item For all $j\in[1,w_2]$, $(v,m_i)\of p^1_{o(j)}\to p^2_j$ [$(v,m_i)\of p^1_{i}\to p^2_{o(j)}$] is an ab-branch reflection [preservation] pre-morphism.
\end{itemize}
%
The component $v$ of an ab-condition pre-morphism $m$ is called its \emph{root arrow} and is denoted $v_m$. $m$ is an ab-condition reflection [preservation] \emph{morphism} if $R_1=R_2=R$ and $v=\id_R$.

\item Let $p_i=(a_i,c_i)\in \AB{R_i}$ for $i=1,2$. An \emph{ab-branch reflection [preservation] pre-morphism} from $p_1$ to $p_2$ is a tuple $(v,m)\of p_1\to p_2$ such that
\begin{itemize}[nosep]
\item $v\of R_2\to R_1$ is an arrow;
\item $m\of c_2\to c_1$ is an ab-condition reflection [preservation] pre-morphism;
\item $a_1$ and $a_2$ together with $v$ and $v_m$ form a reflection [preservation] square.
\end{itemize}
\end{itemize}
\end{definition}
%
Note that if $\depth(c_2)=0$ for the target $c_2$ of some ab-condition morphism $m\of c_1\to c_2$, $o$ is the initial the requirement on the branch morphisms is vacuously fulfilled regardless of the nature of $v$, whereas if $\depth(b)>0$, the root arrow $v_m$ must be at least epi.

\begin{definition}[identities and composition of ab-reflection morphisms]\label{def:ab-reflection composition}
Let $c_i=(R_i,p^i_1\ccdots p^i_{w_i})\in \AC{R_i}$ be ab-conditions for $i=1,2,3$ and let $m_i=(v_i,o_i,m^i_1\ccdots m^i_{w_i}) \of c_i\to c_{i+1}$ be ab-reflection morphisms for $i=1,2$. Then $\id_{c_1}$ and $m_1;m_2$ are defined as follows (where $c^1_j$ is the subcondition of branch $p^1_j$ for $j\in [1,w_1]$):
%
\begin{eqnarray}
\id_{c_1} & =
  & (\id_{R_1},\id_{[1,w_1]},\id_{c^1_1}\ccdots \id_{c^1_{w_1}})
  \eqlabel{id condition morphism} \\
m_1;m_2 & =
  & (v_2;v_1,\, o_2;o_1,\, m^2_1;m^1_{o_2(1)}\cdots m^2_{w_3};m^1_{o_2(w_3)}) \enspace.
 \eqlabel{condition morphism composition}
\end{eqnarray}
\end{definition}

\begin{proposition}[ab-reflection pre-morphisms reflect satisfaction]
  \label{preop:ab-reflection morphisms}
Let $b\in \AC R,c \in \AC{S}$ be arrow-based conditions. If $m:b\to c$ is an ab-condition reflection pre-morphism, then $v_m;g\sat c$ implies $g \sat b$ for all arrows $g\of R\to G$. 
\end{proposition}

\begin{proof}[]
By induction on $\depth(b)+\depth(c)$. Let $b=(R,p_1\cdots p_{|b|})$ such that $p_i=(a^p_i,c^p_i)$ for all $i\in[1,|b|]$ and $c=(S,q_1\cdots q_{|c|})$ such that $q_i=(a^q_i,c^q_i)$ for all $i\in[1,|c|]$, and let $m=(v,o,m_1\ccdots m_{|c|})$.
\begin{description}
\item[Base case] If $\depth(b)+\depth(c)=0$, it follows that $|c|=0$; hence $v_m;g\sat c$ is contradictory.
\item[Induction step.] Assume $v;g\sat c$, due to $v;g\sat q_i$ for some $i\in [1,|c|]$; and let $h$ be the witness, meaning $v;g=a^q_i;h$ and $h\not\sat c^q_i$. Because $(v,m_i)\of p_{o(i)}\to q_i$ is an ab-branch morphism, $a^p_{o(i)}$ and $a^q_i$ together with $v$ and $v_{m_i}$ form a reflection square, hence $g=a^p_{o(i)};v_{m_i};h$.

Recall that $m_i\of c^q_i\to c^p_{o(i)}$ is an ab-reflection morphism and observe that $\depth(c^q_i)+\depth(c^p_{o(i)}) < \depth(b)+\depth(c)$; hence by the induction hypothesis, $m_i$ fulfils the property. If $v_{m_i};h\sat c^p_{o(i)}$ then $h\sat c^q_i$, which is contradictory; it follows that $v_{m_i};h\not\sat c^p_{o(i)}$, from which we may conclude $g\sat p_{o(i)}$ and hence $g\sat b$. \qed
\end{description}
\end{proof}

\begin{proposition}[ab-preservation pre-morphisms preserve satisfaction]
  \label{prop:ab-preservation morphisms preserve}
Let $b\in \AC R,c \in \AC{S}$ be arrow-based conditions. If $m:b\to c$ is an ab-condition preservation morphism, then $g\sat b$ implies $v_m;g \sat c$ for all arrows $g\of R\to G$. 
\end{proposition}
...
\item[Induction step.] Assume $g\sat b$, due to $g\sat p_i$ for some $i\in [1,|b|]$; and let $h$ be the witness, meaning $g=a^p_i;h$ and $h\not\sat c^p_i$. Because $(v,m_i)\of p_{i}\to q_{o(i)}$ is an ab-branch preservation morphism, $a^p_i$ and $a^q_{o(i)}$ together with $v$ and $v_{m_i}$ form a preservation square, hence (due to \cref{prop:preservation square}.\ref{preservation square any}) there is a $h'$ such that $h=v_{m_i};h'$ and $v_m;g=a^q_{o(i)};h'$.

Recall that $m_i\of c^p_i\to c^q_{o(i)}$ is an ab-preservation pre-morphism and observe that $\depth(c^q_i)+\depth(c^p_{o(i)}) < \depth(b)+\depth(c)$; hence by the induction hypothesis, $m_i$ fulfils the property. Therefore, if $h'\sat c^q_{o(i)}$ then $h=v_{m_i};h'\sat c^p_i$, which is contradictory; it follows that $h'\not\sat c^q_{o(i)}$, from which we may conclude $v_m;g\sat q_{o(i)}$ and hence $v_m;g\sat c$. \qed
\end{description}
\end{proof}
