...
The notion of \emph{morphism} of nested conditions has not received much attention in the literature. When considered at all, such as in \cite{bchk:conditional-reactive-systems,sksclo:coinductive-techniques-for-satisfiability}, they are essentially based on the semantics in terms of satisfaction. Indeed, entailment (see Def.~\ref{def:ab-satisfaction}) establishes a preorder over conditions. Let us denote the resulting category $\ABC^{\entails}$, where if $c$ and $b$ are ab-conditions, $c \leq b$ iff $c \entails b$.%
...
The underlying principle for a morphism $m$ from $b$ to $c$ is that it must identify, for every $c$-branch $p^c_i$, a $b$-branch $p^b_j$ that it entails. However, because of their inductively defined tree-shape, on lower levels we have to relate (sub)conditions over different roots; hence, in general $R^c$ and $R^b$ are not the same. Instead, a morphism $m$ will include an arrow $v\of R^c\to R^b$ (in the opposite direction of $m$, and the entailment of $p^b_j$ by $p^c_i$ is \emph{modulo} $v$, in a way made precise below.

\begin{definition}[entailment square]\label{def:entailment square}
Let $a_1\of R_1\to P_1$, $a_2\of R_2\to P_2$, $v\of R_2\to R_1$ and $v'\of P_1\to P_2$, and consider the following (not necessarily commuting) diagram:
\[\begin{tikzpicture}[auto,node distance = 1.5cm,baseline=(a1)]
\node (R1) {$R_1$};
...
\node (G) [below left=.5 and .5 of P1] {$G$};
\path
  (R1) edge [dashed,->,bend right=25] node[left] {$g$} (G)
  (P2) edge [dashed,->,bend left=25] node[auto] {$h$} (G);
\end{tikzpicture}\]
%
The diagram is called an \emph{entailment square} if, for any pair of arrows $g\of R_1\to G$, $h\of P_2\to G$, the $v;g=a_2;h$ implies $g=a_1;v';h$ (in other words, the bottom left subdiagram commutes if the outer subdiagram commutes). 
\end{definition}

\begin{proposition}\label{prop:entailment square}
Consider a diagram as in Def.~\ref{def:entailment square}.
\begin{enumerate}[defsep]
\item If $v$ is epi and the square commutes (i.e., $a_2=v;a_1;v'$), the diagram is an entailment square.
\item If $v$ is split epi with section $s$ such that $s;a_2=a_1;v'$, the diagram is an entailment square.
\item Entailment squares compose; i.e., in
\[\begin{tikzpicture}[auto,node distance = 1.5cm,baseline=(a1)]
\node (R1) {$R_1$};
\node (P1) [below=of R1] {$P_1$};
\node (R2) [right=of R1] {$R_2$};
...
\path
  (R1) edge [->] node (a1) [auto] {$a_1$} (P1)
  (R2) edge [->] node [auto] {$a_2$} (P2)
       edge [->] node [auto] {$v_1$} (R1)
  (P1) edge [->] node [auto] {$v_1'$} (P2)
  (R3) edge [->] node [auto] {$a_3$} (P3)
       edge [->] node [auto] {$v_2$} (R2)
  (P2) edge [->] node [auto] {$v_2'$} (P3);
\end{tikzpicture}\]
if both the left hand and the right hand subdiagrams are entailment squares, then so is the outer subdiagram.
\end{enumerate}
\end{proposition}

\begin{proof}[]
\begin{enumerate}
\item Since the diagram commutes, $v;g=a_2;h=v;a_1;v';h$; hence $g=a_1;v';h$ because $v$ is epi.

\item The diagram becomes
\[\begin{tikzpicture}[auto,node distance = 1.5cm,baseline=(a1)]
\node (R1) {$R_1$};
\node (P1) [below=of R1] {$P_1$};
\node (R2) [right=of R1] {$R_2$};
...
\node (X) [below right=.4 and .5 of R1] {$R_1$};

\path (X) edge [->] node[pos=.4,below left,inner sep=1] {$\id$} (R1)
          edge [->] node[pos=.4,below right,inner sep=1] {$s$} (R2);
\end{tikzpicture}\]
where $s;a_2=\id;a_1;v'$ and $s;v=\id$; hence $v;g=a_2;h$ implies $g=\id;g=s;v;g=s;a_2;h=\id;a_1;v';h=a_1;v';h$.
...
\path
  (R1) edge [dashed,->,bend right=25] node[left] {$g$} (G)
  (P3) edge [dashed,->,bend left=20] node[auto] {$h$} (G);
\end{tikzpicture}\]
If $v_2;v_1;g=a_2;h$, then the entailment square property of the right hand subdiagram implies $v_1;g=a_2;v_2';h$, and hence the entailment square property of the left hand subdiagram implies $g=a_1;v_1';v_2';h$.
\qed
\end{enumerate}
\end{proof}
%
It is an immediate observation that, for all entailment squares obtained by applying Prop.~\ref{prop:entailment square}, the upper arrow $v$ is epi; however, we currently have no proof whether that is a necessary condition. It is certainly \emph{not} the case that every entailment square obtained through composition satisfies either Clause~1 or Clause~2 of the proposition.\todo{Provide example?}

\begin{definition}[arrow-based condition morphism]\label{def:ab-morphism}
\begin{itemize}[defsep]
\item Let $c_i = (R_i,p^i_1\ccdots p^i_{w_i})\in \AC{R_i}$ for $i=1,2$. An \emph{ab-condition morphism (or ab-morphism)} $m\of c_1 \to c_2$ is a tuple $(v,o,m_1\ccdots m_{w_2})$ such that
\begin{itemize}[nosep]
\item $v\of R_2\to R_1$ is an arrow;
\item $o\of [1,w_2]\to [1,w_1]$ is a mapping from $c_2$-branches to $c_1$-branches;
\item For all $j\in[1,w_2]$, $(v,m_i)\of p^1_{o(j)}\to p^2_j$ is an ab-branch morphism.
\end{itemize}
%
The component $v$ of an ab-condition morphism $m$ is called its \emph{root arrow} and is denoted $v_m$.

\item Let $p_i=(a_i,c_i)\in \AB{R_i}$ for $i=1,2$. An \emph{ab-branch morphism} from $p_1$ to $p_2$ is a tuple $(v,m)\of p_1\to p_2$ such that
\begin{itemize}[nosep]
\item $v\of R_2\to R_1$ is an arrow;
\item $m\of c_2\to c_1$ is an ab-condition morphism;
\item $a_1$ and $a_2$ together with $v$ and $v_m$ form an entailment square.
\end{itemize}
\end{itemize}
\end{definition}
%
...
Let $c_i=(R_i,p^i_1\ccdots p^i_{w_i})\in \AC{R_i}$ be ab-conditions for $i=1,2,3$ and let $m_i=(v_i,o_i,m^i_1\ccdots m^i_{w_i}) \of c_i\to c_{i+1}$ be ab-condition morphisms for $i=1,2$. Then $\id_{c_1}$ and $m_1;m_2$ are defined as follows (where $c^1_j$ is the subcondition of branch $p^1_j$ for $j\in [1,w_1]$):
%
\begin{eqnarray}
\id_{c_1} & =
...
\begin{proposition}[ab-condition morphisms reflect models]
  \plabel{ab-morphisms reflect models}
Let $b\in \AC R,c \in \AC{S}$ be arrow-based conditions. If $m:b\to c$ is an ab-condition morphism, then $v_m;g\sat c$ implies $g \sat b$ for all arrows $g\of R\to G$. 
\end{proposition}
...
\item[Induction step.] Assume $v;g\sat c$, due to $v;g\sat q_i$ for some $i\in [1,|c|]$; and let $h$ be the witness, meaning $v;g=a^q_i;h$ and $h\not\sat c^q_i$. Because $(v,m_i)\of p_{o(i)}\to q_i$ is an ab-branch morphism, $a^p_{o(i)}$ and $a^q_i$ together with $v$ and $v_{m_i}$ form an entailment square, hence $g=a^p_{o(i)};v_{m_i};h$.

Recall that $m_i\of c^q_i\to c^p_{o(i)}$ is an ab-morphism and observe that $\depth(c^q_i)+\depth(c^p_{o(i)}) < \depth(b)+\depth(c)$; hence by the induction hypothesis, $m_i$ fulfils the property. If $v_{m_i};h\sat c^p_{o(i)}$ then $h\sat c^q_i$, which is contradictory; it follows that $v_{m_i};h\not\sat c^p_{o(i)}$, from which we may conclude $g\sat p_{o(i)}$ and hence $g\sat b$. \qed
\end{description}
\end{proof}
