...
\end{tikzpicture}\]
if both the left hand and the right hand subdiagram are entailment squares, then so is the outer subdiagram.
\end{enumerate}
\end{proposition}
...
\end{tikzpicture}\]
If $v_2;v_1;g=a_2;h$, then the entailment square property of the right hand subdiagram implies $v_1;g=a_2;v_2'$, and hence the entailment square property of the left hand subdiagram implies $g=a_1;v_1';v_2';h$.
\qed
\end{enumerate}
\end{proof}
%
...
\item Let $p_i=(a_i,c_i)\in \AB{R_i}$ for $i=1,2$. An \emph{ab-branch morphism} from $p_1$ to $p_2$ is a tuple $\tupof{v,m}\of p_1\to p_2$ such that
\begin{itemize}[nosep]
\item $v\of R_2\to R_1$ is an arrow;
...
\end{definition}
%
Note that if $\depth(b)=0$ for the target $b$ of some ab-condition morphism $m\of c\to b$, the requirement on the branch morphisms is vacuously fulfilled, whereas if $\depth(b)>0$, the root arrow $v_m$ must be at least epi.
...
\item[Induction step.] Assume $v;g\sat c$, due to $v;g\sat q_i$ for some $i\in [1,|c|]$; and let $h$ be the witness, meaning $v;g=a^q_i;h$ and $h\not\sat c^q_i$. Because $(v,m_i)\of p_{o(i)}\to q_i$ is an ab-branch morphism, $a^p_{o(i)}$ and $a^q_i$ together with $v$ and $v_{m_i}$ form an entailment square, hence $g=a^p_{c(i)};v_{m_i};h$.
