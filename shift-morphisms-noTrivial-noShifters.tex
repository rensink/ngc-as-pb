...
The notion of \emph{morphism} of nested conditions has not received much attention in the literature. When considered at all, such as in \cite{bchk:conditional-reactive-systems,sksclo:coinductive-techniques-for-satisfiability}, they are essentially based on the semantics in terms of satisfaction. Indeed, entailment (see \dcite{ab-satisfaction}) establishes a preorder over conditions. Let us denote the resulting category $\ABC^{\entails}$, where if $c$ and $b$ are ab-conditions, $c \leq b$ iff $c \entails b$.%
...
It also implies that morphisms should only relate conditions with the same root, because entailment is defined only among conditions over the same root (see \dcite{ab-satisfaction}).\todo{AC: rephrase} Technically, the main result of this section will be formulated categorically as the existence of  contravariant functors from  categories of conditions with structural morphisms to $\ABC^{\entails}$.
...
Given two conditions $c\in \AC{A}$ and $b\in \AC{B}$ and an arrow $v\of A \to B$, we have two main ways to reduce both to the same root, using \dcite{shift}. We can either precompose $b$ with $v$ obtaining $\bshift{b}{v}\in \AC{A}$, or we can shift $c$ along $v$, obtaining $\shift{c}{v}\in \AC{B}$. As commented earlier, precomposition is a local operation that only affects the top-level branches of a condition, while shifting is a global operation that affects the branches of a condition at any depth. In the definition of structural morphisms between ab-conditions the roots of subconditions have to be reconciled at each level: this makes the use of shift quite complex to manage. Therefore we will stick to precomposition.\todo{AC: add discussion on shift-related morphisms, when the relationship with span-based ones is more clear, or leave their study to future work.} 
...
\begin{definition}[split epi, split mono, retraction, section]\dlabel{retraction}
...
\begin{definition}[forward and backward shifters]\dlabel{ab-shifter}
...
Morphisms involving conditions of depth 0 enjoy some relevant properties, that we will exploit later. We also show that the disjunctions in clauses 3 and 4 of \dcite{ab-morphism} do not introduce ambiguity. 
...
\item 
The disjunction in clause 3  of \dcite{ab-morphism} is not ambiguous, because if $c_{o(i)}$ has depth 0 and there is a forward shifter $\cF_i$ for $v_i$,  then 
$m_i: b_i \to \cF_i(c_{o(i)})$ is the terminal morphism.
\item 
The disjunction in clause 4  of \dcite{ab-morphism} is not ambiguous, because if $c_{o(i)}$ has depth 0 and there is a backward shifter $\cB_i$ for $v_i$, then $m_i$ is the terminal morphism $!_{\bshift{b_i}{v_i}}: \bshift{b_i}{v_i} \to c_{o(i)}$.
\end{enumerate}
\end{lemma}
...
\begin{proof}
\begin{enumerate}
  \item For clause 1 of \dcite{ab-morphism} there is a unique function $o: \varnothing \to [1,|b|]$, and the other clauses of are vacuous.
  \item If $c$ has depth 0 set $[1, |c|]$ is empty, thus the function $o$ of clause 1 of \dcite{ab-morphism} is well defined only if $|b| = 0$, which also implies that $b=c$.
...
\begin{definition}[identities and composition of ab-morphisms]\dlabel{ab-morphism-composition}
...
\end{enumerate}
%
The other two clauses are proved by induction on the depth of $m$ and $n$. For clause 1 of \dcite{ab-morphism}, let $o=o^n;o^m$, which is a function from $[1,w]$ to $[1,|e|]$ as needed. For clause 2, let $v_i=v^m_{o^n(i)};v^n_i$ for all $1\leq i\leq w$. All $v_i$ are arrows from $P^e_{o(i)}$ to $P^b_i$, and they commute with the arrows from the root, as required. In fact, $a^e_{o(i)};v_i =\text{(def.~of $v_i$) } a^e_{o(i)};v^m_{o^n(i)};v^n_i  =\text{($m$ is a morphism) }  a^c_{o^n(i)} ; v^n_i =\text{($n$ is a morphism) }  a^b_i$.
%
\begin{enumerate}[resume]
\item
For clause 3 of \dcite{ab-morphism} we have to show that for all $1\leq i\leq w$, either $e_{o(i)}$ has depth 0 and $n_i;m_{o^n(i)}: b_i \to \zeroCond{P^b_i}$ is the terminal morphism, or there is a forward root shifter $\cF_i$ for $v_i$ such that $n_i;m_{o^n(i)}:b_i \to \cF_i(e_{o(i)})$ is a forward-shift morphism. 
...
If $e_{o(i)}$ has depth 0, then $m_{j}:c_j \to \zeroCond{P^c_j}$ is the terminal morphims, by \dcite{ab-morphism} and clause 3 of \lcite{terminal morphisms}. If $n_i:b_i \to \cF^n_i(c_{j})$ is a forward shift morphism then we obtain 
...
Let $\ABC^{\forw}$  and  $\ABC^{\back}$ be the categories defined in \dcite{ab-morphism categories}. There are identity-on-objects functors $\op{(\ABC^{\forw})} \to \ABC^{\entails}$ and $\op{(\ABC^{\back})} \to \ABC^{\entails}$.
\end{theorem} 
%
