...
\path
  ...
  (AC-entails.south west) edge[->]
                   node (N) {}
                   node[left,pos=.35] {$\cN$}
                   node[left,pos=.65] {(\thcite{ab-to-sb functors})} (SC-entails.north west)
  (SC-entails.north east) edge[->] 
                   node (A) {}
                   node[right,pos=.65] {$\cA$} 
                   node[right,pos=.35] {(\thcite{sb-to-ab functor})} (AC-entails.south east)
