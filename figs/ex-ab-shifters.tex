...
\begin{tikzpicture}[on grid]
  ...
  \tri[green]{A}{2.6}{1.7}{3.4}{}
  \node (labc) [above of=A-left, xshift=5mm, yshift=5mm] {$c$};
  \node[graph,below=2 of A] (C) {\Cgraph};
  \node [above right=0 and 0 of C.north west] {};
  \node[graph] (B) [right=2.3 of A] {\oneedge{x'}{a}{z'}};
  \tri[red]{B}{2.6}{-0.6}{3.4}{}
  \node (labF) [above of=B-right, xshift=10mm, yshift=5mm] {$\cF_v^{s_1}(c) $};
  ...
  \path (B.south) edge[->] 
  % node[near start,below right,inner sep=0pt] {$\cF_v^{s_1}(c) $}  
...
\begin{tikzpicture}[on grid]
  ...
  \path (A.south) edge[->] node[near start,below left=-.1] {$\cB_{v'}(a)$}
                           node[below left] 
            {$\mapping{
			x' & x \\
			y' & z \\
