\begin{tikzpicture}[>=latex,on grid]
  ...
  \node (Sd2) [below left=1.5 and 1 of Rd] {$R_{d_x}$};
  \node (Sd3) [below=1 of Rd] {$\cdots$};
  \node (Sd4) [below right=1.5 and 1 of Rd] {$R_{d_y}$};
  \node (Sd5) [below right=1 and 1.5 of Rd] {$\cdots$};
  \node (d2) [triangle,below=.2 of Sd2.center] {$d_x$};
  \node (d4) [triangle,below=.2 of Sd4.center] {$d_y$};

  \path (Rd) edge[->] node[above left] {$r^d_x$} (Sd2)
        (Rd) edge[->] node[above right] {$r^d_y$} (Sd4);
