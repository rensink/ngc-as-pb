...
In order to relate conditions at different roots, we introduce a special kind of mapping called a \emph{root shifter}.
\iffull
which will later\todo{AR: not yet...\\ AC: For the version to be submitted now I would delete the sentence and add this as future work. Maybe in the weekend we are able to add this result (which does not require too much).} turn out to be functors between categories of conditions. For now, we note that t
\fi
The purpose of root shifters is to relate the models of a condition $b$ to those of a condition $c$, given an arrow $v:R^b\to R^c$ between their roots (the existence of which implies that the model sets are typically disjoint). To be more precise, we will require that \emph{every $v$-prefixed model of $b$ induces a model for $c$}: that is, if $v;g$ is a model of $b$, then $g$ is a model of $c$.
...
\end{example}
%
\pcite{ab-morphisms preserve models} can be phrased more abstractly as the existence of a functor from a category of arrow-based conditions and their (forward-shift or backward-shift) morphims to $\ABC^{\entails}$. An important step to arrive at that result is to show that morphims compose. This, in turn, depends on the preservation of morphisms by root shifters.\todo{AR: This is actually the core of the claimed (but not stated) property that root shifters give rise to functors.\\ AC: I agree}
