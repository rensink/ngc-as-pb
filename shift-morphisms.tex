...
\end{example}
%
\pcite{ab-morphisms preserve models} can be phrased more abstractly as the existence of a functor from a category of arrow-based conditions and their (forward-shift or backward-shift) morphims to $\ABC^{\entails}$. An important step to arrive at that result is to show that morphims compose. This, in turn, depends on the preservation of morphisms by root shifters.\todo{AR: This is actually the core of the claimed (but not stated) property that root shifters give rise to functors.}
