...
\todo{AR: Added to ease the definition of the indexed categories.}Each of these categories $\ABC^{\mathsf x}$ actually consists of disjoint full subcategories $\ABC^{\mathsf x}(X)$ of $X$-rooted conditions.
\end{theorem}
%
...
\end{example}
%
As a final observation, we can characterise the relation between shifters and conditions as a special construction called an \emph{indexed category}. For this purpose, let $\bC^\shift$ denote the category with objects from $\bC$ (which can be any presheaf topos) and arrows $\cS\of X\to Y$ whenever $\cS$ is a source shifter from $Y$ to $X$.

Moreover, for every $\bC^\shift$-object $X$ let $\ACx^\forw(X)=\ABC^\forw(X)$, and for every $\bC^\shift$-arrow $\cS\of X\to Y$ let $\ACx^\forw(\cS)$ be the functor that maps every $c\in \AC Y$ to $\bar\cS(c)\in \AC X$ and every forward-shift $m\of b\to c$ (for $b,c\in \AC Y$) to $m\of \bar\cS(b)\to \bar\cS(c)$. Define $\ACx^\back$ analogously for backward-shift morphisms.

\begin{proposition}[$\bC^\shift$-indexed categories]\plabel{ab indexed}
$\bC^\shift$ is a well-defined category, and for all $\cS\of X\to Y$, $\ACx^\forw(\cS)$ and $\ACx^\back(\cS)$ are well-defined functors from $\ABC^\forw(Y)$ to $\ABC^\forw(X)$ and from $\ABC^\back(Y)$ to $\ABC^\back(X)$, respectively. Moreover, $\ABx^\forw$ and $\ACx^\back$ are functors from $(\bC^\shift)^\op$ to $\Cat$, establishing $\bC^\shift$-indexed categories.
\end{proposition}
