...
\begin{definition}[arrow-based condition]\dlabel{ab-condition}
  ...
  \begin{itemize}
  \item $c\in \AC R$ if $c=(R,\cP)$ where $\cP\subseteq \AB R$ is a finite set;
...
Note that, as a consequence of the inductive nature of \dcite{ab-condition}, every ab-condition has a finite \emph{depth} $\depth(c)$, defined as $0$ if $\cP_c=\emptyset$ and $1+\max_{p\in \cP_c} \depth(c_p)$ otherwise.
