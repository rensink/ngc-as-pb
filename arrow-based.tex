...
\begin{definition}[arrow-based condition morphism]\dlabel{ab-morphism}
  ...
  % from $c$ to $d$ 
  is a pair $(t,\setof{(q_p,m_p)}_{p\in \cB_c})$ such that   
  \begin{itemize}
    \item $t:R_c\func R_d$ is an arrow, and  
  \item  for all branches $p\in \cB_c$, 
  \begin{itemize}
    \item $q_p$ is a branch of $d$, i.e.~ $q_p \in \cB_d$,
    \item $m_p:c_{q_p}\func c_p$ is an ab-condition morphism,
  \end{itemize}
    
    and there is a section $s:R_d\func R_c$ for $t$ such that $s; r_p = r_q;t_{m_p}$.
...
\begin{figure}
  ...
  \caption{Pictorial representation of an arrow-based condition morphism $m:c\func d$. Here $c=(R_c, \setof{(r^c_i,c_i)}_{1\leq i\leq |c|})$, $d=(R_d, \setof{(r^d_i,d_i)}_{1\leq i\leq |d|})$, and     
  $m = (t,\setof{(q_i,m_i)}_{1\leq i \leq |c|})$, where $q_1 = (r^d_x,d_x), \ldots,q_n = (r^d_y,d_y)$. For each $1 \leq i \leq |c|$, $s_i$ is the section of $t$ required by the definition.}
  %  is the $i$-th branch of $d$, and $q_n$ is the $j$-th. 
  %  $p^d_i = q_{p^c_1}$ and $p^d_j = q_{p^c_n}$ }
