...
% The definitions below use \emph{objects} and \emph{arrows} as building blocks. These can for now be thought of as the objects and arrows of a generic category $\bC$; however, our examples and intuitions will all be based on $\cat{Graph}$,\todo{AC: formal def. of the category in background section?} the category of directed, edge-labelled multigraphs. 
...
%
%\emph{Terminology and notation.}
If $g\sat c$, we also say that $g$ is a \emph{model} for $c$. We call $p$ the \emph{responsible branch} and $h$ the \emph{witness} for $g\sat c$. Pictorially, $g\sat c$ with responsible branch $p_i$ and witness $h$ can be visualised as in \fcite{ab-satisfaction}.
%
\begin{figure}
  \centering
