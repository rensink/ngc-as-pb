...
%
\begin{proposition}\plabel{morphism preserves satisfaction}
...
%
\begin{enumerate}
\item\exlabel{no pullback} $\phi\equiv \exists y\st a(x,y)$ entails $\psi\equiv \exists z,y\st a(z,y)$. Note the discrepancy of the free variables. The decomposed condition for $\psi$ must have $x$ in its root for a morphism to exist; however, the interface ``loses'' that $x$. See \fcite{no pullback} for a visualiation.
