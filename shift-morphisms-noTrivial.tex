...
\todo{AC: A source shifter $S$ is a pre-composition function, mapping $f$ to $k;f$. A forward source shifter for $v:A \to B$ pre-composes with $S(id_A)$, which is a section of $v$ (thus $v$ is split-epi). A backward source shifter for $v:A \to B$ pre-composes with $v$, that must be epi. Use this as definition? Then all the rest follows...\\ 
How this relates to precomposition of Def. 2.4?}
...
%
\begin{proof}
Let $\cC=\bar\cS$ for a source shifter $\cS$. Let $v=\cS(\id_X)$; then \lcite{ab-source shifters} implies $\cS(a)=v;a$ for any $a$ with source $X$.\qed
\end{proof}
\end{fullorname}
%
...
\item Let $b=\cB(c)$; then $a^b_i=\cB(a^c_i)$ for all $1\leq i\leq |c|$. Assume $v;g\sat b$ due to responsible branch $p^b_i$ and witness $h$; i.e., $v;g=\cB(a^c_i);h$. \eqcite{backward condition} then implies $g=a_i;h$, hence $p^c_i$ is a responsible branch and $h$ a witness for $g\sat c$.
\qed
\end{enumerate}
\end{proof}
\end{fullorname}
...
\begin{enumerate}[topsep=\smallskipamount]
\item For every $X$, the identity function on $\cat{C}(X,\_)$, denoted $\cI_X$ and called the identity shifter, is a source shifter that is forward and backward for $\id_X$.\todo{$\cI_X$ is pre-composition with the identity, which is split mono and epi.}
\item If $\cU$ is a source [root] shifter from $X$ to $Y$ and $\cV$ a source [root] shifter from $Y$ to $Z$, then $\cU;\cV$ is a source [root] shifter from $X$ to $Z$. \todo{AC: source shifter is precomposition with an arrow. Just compose the two arrows.}
\item If $\cU$ and $\cV$ are forward source [root] shifters for $u: X\to Y$ and $v: Y \to Z$, respectively, then $\cU;\cV$ is a forward source [root] shifter for $u;v$.\todo{AC: split monos compose}
\item Dually, if $\cU$ and $\cV$ are backward source [root] shifters for $u: Y\to X$ and $v: Z \to Y$, respectively, then $\cU;\cV$ is a backward source [root] shifter for $v;u$.\todo{AC: epis compose}
\end{enumerate}
\end{proposition}
...
\begin{fullorname}
\begin{proof}
Assume $m = (o,(v_1,m_1)\cdots(v_{|b|},m_{|b|}))$ and let $v$ be as in \lcite{ab-root shifters}. The only new proof obligation for $m$ to be an ab-morphism from $c$ to $b$ is that, for all $1\leq i\leq |b|$, $v;a^c_{o(i)};v_i=v;a^b_i$. This follows immediately from $a^c_{o(i)};v_i=a^b_i$.\qed
\end{proof}
\end{fullorname}
%
