...
\end{figure}
%
As we will see later, we can reconstruct an arrow-based condition from a span-based one by taking pushouts over the spans $\spanof{u_i}{d_i}$; the arrows $a_i$ will then correspond to the image of $d_i$ in the pushout cospan.\todo{AR: Add definition or remove sentence, modify description of example accordingly}

As an example of span-based conditions, \fcite{sb-conditions} shows the counterparts of the three arrow-based conditions in \fcite{ab-conditions}.\todo{add third} The original ones can be reconstructed from the span-based case...
...
\end{description}
%
This gives rise to the following four candidate shifters (which still have to be shown to satisfy the conditions of \dcite{sb-shifters}):
%
\begin{align*}
...
%
\iffest
We also observe that span shifters compose, exactly as
shown for arrow shifters in \pcite{ab-shifters compose}. This also holds for complete shifters. The full statement is given in \pcite{sb-shifters compose}.
\else
...
\begin{itemize}[topsep=\smallskipamount]
\item If $\cS$ is a forward shifter, then $\cS = \cS_1;\cdots;\cS_n$ where each $\cS_i$ is an elementary forward shifter for some $v_i$ such that $v=v_1;\cdots;v_n$. If, moreover, $\cS$ is complete then either $\cS$ is trivial or all $\cS_i$ are complete (in which case, in fact, $\cS_i=\dF{v_i}$ for all $i$ and $\cS=\dF v$).
\item If $\cS$ is a backward shifter, then $\cS = \cS_1;\cdots;\cS_n$ where each $\cS_i$ is an elementary backward shifter for some $v_i$ such that $v=v_n;\cdots;v_1$. If, moreover, $\cS$ is complete then either $\cS$ is trivial or all $\cS_i$ are complete (in which case, in fact, $\cS_i=\dB{v_i}$ for all $i$ and $\cS=\dB v$).
\end{itemize}
\end{assumption}
%
...
Consider the categories having sb-conditions as objects and different arrows: 
$\SBC^\forw$ with forward-shift morphims, $\SBC^{\back}$ with backward-shift morphisms, $\SBC^{\forw\circ}$ with complete forward-shift morphisms, $\SBC^{\entails}$ with the preorder of entailment, and $\SBC^\equiv$ with the preorder (actually, an equivalence) of semantic equivalence. These are well-defined, and there are identity-on-objects functors $\SBC^\forw \to \SBC^{\entails}$, $\SBC^\back \to \SBC^{\entails}$ and $\SBC^{\forw\circ} \to \SBC^{\equiv}$.
\end{theorem}

\subsection{From arrow-based to span-based conditions}

\begin{itemize}
\item The construction of sb-conditions from ab-conditions
