...
\end{equation}
%
A forward span source shifter is called \emph{complete}\todo{other term?} if it also reflects models adding $v$; i.e., if the ``implies" in \eqref{eq:sb-forward condition} is an ``if and only if".
...
\end{description}
%
These options are depicted in \fcite{sb-source shifters} (in a simplified construction that omits the intermediate steps of the span composition where possible). It turns out that, indeed, each of these four options gives rise to a valid shifter.
%
\begin{figure}
\centering
...
Let $v:P_1\to P_2$ be an arrow, and for $i=1,2$ let $\spanof{u_i}{d_i}:R\to P_i$ be spans with interface objects $I_i$. We say that $(s_1,s_2)$ is a \emph{pattern shift pair for $v$} if there is a span $\spanof{k_1}{k_2}:I_1\to I_2$ with $k_1$ epi, such that both halves of the following diagram commute:
\begin{center}
\begin{tikzpicture}[on grid]
\node (R) {$R$};
\node (I1) [below left=1 and 1.5 of R] {$I_1$};
\node (P1) [below=1.5 of I1] {$P_1$};
\node (I2) [below right=1 and 1.5 of R] {$I_2$};
\node (P2) [below=1.5 of I2] {$P_2$};
\node[red] (J) [below=of R] {$J$};
\path
  (I1) edge[->] node[above left,inner sep=1] {$u_1$} (R)
  (I1) edge[->] node[left] {$d_1$} (P1)
  (I2) edge[->] node[above right,inner sep=1] {$u_2$} (R)
  (I2) edge[->] node[right] {$d_2$} (P2);
\path[morphism]
  (P1) edge[->] node[above] {$v$} (P2);
\path[red,color=red]
  (J) edge[->>] node[near start,below] {$k_1$} (I1)
  (J) edge[->] node[near start,below] {$k_2$} (I2);
\end{tikzpicture}
\end{center}
We say that the pattern shift pair is \emph{conservative} if, in addition, the lower square is a pushout (i.e., a colimit of the span $(k_1;d_1,d_2)$. The set of pattern shift pairs under $v$ is denoted $\cP_v$, and the set of conservative pattern shift pairs $\ccP_v$.
\end{definition}
%
The following lists some essential properties of pattern shifting.

\begin{proposition}[pattern shift properties]
Let $v:P_1\to P_2$ and $t:P_2\to P_3$ be arrows, and let $(s_1,s_2)\in \cP_v$.
...
\item\label{pattern-transitive} $(s_1,s_3)\in \cP_{v;t}$ for all $(s_2,s_3)\in \cP_t$;
\item\label{pattern-congruence} $(s;s_1,s;s_2)\in \cP_v$ for all spans $s$ for which the composition is defined.
\end{enumerate}
...
\item\label{conservative-transitive} $(s_1,s_3)\in \ccP_{v;t}$ for all $(s_2,s_3)\in \ccP_t$;
\item\label{conservative-congruence} $(s;s_1,s;s_2)\in \ccP_v$ for all spans $s$ for which the composition is defined.
\end{enumerate}
\end{proposition}
%
...
\begin{definition}[span-based (complete) condition morphism]\dlabel{sb-morphism}
  ...
  \end{itemize}
  $m$ is called \emph{complete} if $o$ is surjective, $(s^b_i,s^c_{o(i)})\in \ccP_{v_i}$ for all $1\leq i\leq |b|$, and $\cF_i$ [$\cB_i$] is complete.
\end{definition}
%
Given the number of possible forward and backward shifters we identified, it would seem that there is a lot of choice within the concept of a morphism, possibly making them complex to reason about. However, the following result states that we actually only have to consider forward shifting.
%
\begin{proposition}\plabel{sb-forward only}
Let $b,c$ be sb-conditions. If $m$ is a [complete] backward- or forward-shift morphism from $b$ to $c$, then $m$ is a [complete] forward-shift morphism from $b$ to $c$ using only $\cF_v^\circ$.
\end{proposition}
%
...
\begin{lemma}~
\begin{enumerate}
\item If $m$ is a [complete] backward-shift morphism from $b$ to $\cB_v^\circ(c)$, then $m$ is a [complete] forward-shift morphism from $\cF_v^\circ(b)$ to $c$.
\item If $m$ is a backward-shift morphism from $b$ to $\cB_v^s(c)$, then $m$ is a forward-shift morphism from $\cF_v^\circ(b)$ to $c$.
\item If $m$ is a forward-shift morphism from $\cF_v^s(b)$ to $c$, then $m$ is a forward-shift morphism from $\cF_v^\circ(b)$ to $c$.
\end{enumerate}
\end{lemma}
%
\begin{proof}
...
\end{proof}
%
Motivated by \pref{sb-forward only}, from now on we drop the qualifiers ``forward-shift" and ``backward-shift" and just talk about (sb-)morphisms, always meaning those that are based on $\cF^\circ$.

As before, we want morphisms to satisfy certain ``nice" properties: model preservation, model reflection in case of completeness, and compositionality. As it turns out, these all follow from the nature of root shifters and pattern shift.

\begin{proposition}[sb-condition morphisms preserve models]
  \plabel{sb-morphisms preserve models}
Let $b,c \in \SC{R}$ be span-based conditions. If $m:b\func c$ is an ab-morphism, then $g\sat b$ implies $g \sat c$ for all arrows $g:R\func G$. Moreover, if $m$ is complete, then also $g\sat c$ implies $g\sat b$ for all $g$.
\end{proposition}
...
\begin{comment}
 However, before embarking on the proofs, we will first two concrete pattern shift relations. For $i=1,2$ let $\spanof{u_i}{d_i}:R\to P_i$ be spans with intermediate objects $I_i$, and let $v:P_1\to P_2$.
