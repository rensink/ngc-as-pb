...
The result is a span based condition, and it is easily shown that for each $g:R \to G$ we have that $g \sat c$ with witness $h$, by \dcite{ab-satisfaction}, if and only if $g \sat \cJ(c)$ with the same witness, by \dcite{sb-satisfaction}. This embedding will be turned into a functor after defining morphisms of span-based conditions.\todo{to be checked.} 
...
\end{definition}
%
Span source shifters extend to root shifters (of span-based conditions) exactly as in the arrow-based case.\todo{AC: expanded later. Delete this sentence?} Before showing how this gives rise to morphisms, however, let us investigate the possible instantiations of span source shifters.
...
\end{proposition}
%
Span root shifters satisfy the analogous properties of arrow root shifters formulated in Propositions \pref{ab-root shifters preserve} and \pref{ab-shifters compose}, extended with the case of complete shifters. As the proofs are essentially identical, we omit them here.\todo{AR: do you agree? AC: yes}
...
\end{proposition}
%
Because we can also obtain span (source and root) shifters by composition, it is not necessarily the case that every forward shifter for $v:A\to B$ is of the shape $\dcF v$, $\rcF v x$ or $\cI_{A,B}$. However, for the remainder of this paper we will restrict ourselves to span shifters that are at least \emph{composed} from these elementary cases, in the following sense:\todo{Discuss}
%
\newtheorem{assumption}{Assumption}
\begin{assumption}[universe of span shifters]
Every span shifter $\cS$ is equal to a sequence $\cS_1;\cdots;\cS_n$ of elementary span shifters $\cS_i$, such that, moreover, all $\cS_i$ are [forward/backward/complete] if $\cS$ is [forward/backward/complete].
\end{assumption}
...
\end{definition}
%
Note that only direct (forward or backward) root shifters are complete, hence also only direct morphisms can be complete.

One of the most essential properties of morphisms is that they preserve models and, in the case of complete morphisms, also reflect them. This is the analogous property of \pcite{ab-morphisms preserve models}, extended with the case for completeness.
%
...
\emph{Provable entailment}, denoted $\pentails$, is the smallest partial order\todo{AC: preorder?} such that for all sb-conditions $b,c$:
\begin{itemize}[topsep=\smallskipamount]
...
\end{itemize}
\emph{Provable equivalence}, denoted $\pequiv$, is defined as ${\pequiv}=({\pentails}\cap{\pentailedby})^*$.\todo{AC: is * necessary?}
\end{definition}
%
The following is a property that we will need in the proof below.
%
\begin{lemma}\llabel{pequiv}
...
%
\begin{proposition}[provable entailment]
...
\end{proposition}
%
This is proved by induction on the depth of the morphism; to make the induction work, we actually need a stronger version of the property.\todo{Already \pcite{sb-forward only} is quite technical, I am not sure it makes sense to leave it in.}
%
\begin{lemma}\llabel{sb-forward only}
Let $b\in \SC A,c\in \SC B$ be sb-conditions, and let $v:A\to B$ be an arrow.
\begin{enumerate}[topsep=\smallskipamount]
\item For any [complete] forward root shifter $\cF$ for $v$, if $m$ is a [complete] forward-shift morphism from $\cF(b)$ to $c$, then $m$ is also a direct [complete] forward-shift morphism from $\dcF v(b)$ to $c$.

\item For any [complete] backward root shifter $\cB$ for $v$, if $m$ is a [complete] backward-shift morphism from $b$ to $\cB(c)$, then there are sb-conditions $b'\pequiv b$ and $c'\pequiv c$ such that $\bar m$ is a direct [complete] forward-shift morphism from $\dcF v(b')$ to $c'$.
\end{enumerate}
\end{lemma}
%
Clearly, \pcite{sb-forward only} is a special case of \lcite{sb-forward only} for $v=\id$.
%
\begin{proof}
Let $b\in \SC A,c\in \SC B$ be sb-conditions, and let $v:A\to B$ be an arrow. The proof is by induction on the depth of $m$.
\begin{enumerate}[topsep=\smallskipamount]
\item Let $\cF$ be a forward root shifter for $v$, and $m=(o,(v_1,m_1)\ccdots(v_w,m_w)):\cF(b)\to c$ a forward-shift morphism. It follows that all $m_i$ are morphisms from $\cF_i(c_{o(i)})$ to $b_i$ for some forward shifter $\cF_i$; by the induction hypothesis therefore, also $m_i:\dcF{v_i}(c_{o(i)})\to b_i$. It only remains to be shown that, for all $1\leq i\leq w$, $(\cF_i(s^c_{o(i)}),s^b_i)\in \cP_{v_i}$ implies $(\dcF{v_i}(s^c_{o(i)}),s^b_i)\in \cP_{v_i}$. This is a consequence of \lcite{BF}.

\smallskip
If $m$ is complete, then all its root shifters are complete, and hence already direct.

\item Let $\cB$ be a backward root shifter for $v$, and let $m=(o,(v_1,m_1)\ccdots(v_w,m_w)):\to \cB(c)$ a backward-shift morphism. It follows that all $m_i$ are morphisms from $c_{o(i)}$ to $\cB_i(b_i)$ for some backward shifter $\cB_i$. By the induction hypothesis therefore (letting $j=o(i)$), $\bar m_i:\dcF{v_i}(c_j')\to b_i'$ for some $c_j'\pequiv c_j$ and $b'_i\pequiv b_i$.
...
c'=\;&(B,p_1\ccdots p_w\,p^c_{j_1}\ccdots p^c_{j_n}) \\
   & \quad\text{where} p_i = (\dcF v(\cB(s^c_{o(i)})),c'_{o(i)}) \text{ for all $1\leq i\leq w$}\enspace.
\end{align*}
%
Note that $|c'|=w+n$. For $1\leq i\leq w$, the $i$th branch of $c'$ is based on the $o(i)$th branch of $c$, modifying its span by first backward-shifting it using $\cB$ and then forward-shifting using $\dcF v$, and modifying its subcondition by taking, instead of $c_{o(i)}$, the provably equivalent $c'_{o(i)}$. For $w+1\leq i\leq w+n$, the $i$th branch of $c'$ is the $j_i$th branch of $c$.

\smallskip
$\bar m$ (which has $o^{\bar m}=\id_{[1,w]}$) is a direct forward-shift morphism from $\dcF v(b')$ to $c'$. To show this, all that is yet required is that $(\dcF v(s^b_i),\dcF v(\cB(s^c_{o(i)}))) \in \cP_{v_i}$ for $1\leq i\leq w$. This follows from the fact that $m:b\to \cB(c)$ is a morphism, which means $(s^b_i,\cB(s^c_{o(i)}))\in \cP_{v_i}$, in combination with \pcite{pattern shift}.\ref{pattern-congruence}, which states that the action of $\dcF v$ on spans (which comes down to prefixing them with $\spanof{v}{\id}$, see \fcite{sb-source shifters}) preserves $\cP_{v_i}$.
...
\end{align*}
\begin{itemize}
\item $c'\pequiv c''$ because the identity morphism $\id_{c'}$ is a direct complete forward-shift morphism from $c'$ to $c''$ due to $(s^c_{o(i)},\dcF v(\cB(s^c_{o(i)})))\in \ccP_\id$ for all $1\leq i\leq w$ (see \lcite{BF});
...
\qed
\end{itemize}
If $m$ is complete, then so is $\bar m$, because in that case $n=0$ (there are no $c$-branches that are not the image of any $b$-branch) and so $\id_{[1,w]}$ is surjective in $\bar m:\dcF v(b')\to c'$.
\end{enumerate}
\end{proof}
%
...
\begin{enumerate}[topsep=\smallskipamount]
\item For any forward shifter $\cF$ for $v$ and any $A$-sourced span $s$, $(\cF(s_1),s_2)\in \cP_v$ implies $(\dcF v(s_1),s_2)\in \cP_v$.
\item For any backward shifter $\cB$ for $v$ and any $B$-sourced span $s$, $(\dcF v(\cB(s)),s)\in \ccP_\id$.
\end{enumerate}
\end{lemma}
%
\begin{proof}
