...
\end{proposition}
%
This is actually a consequence of \pcite{A preserves morpisms} in combination with \lcite{N-A} below. Morphisms remain essentially identical under $\cN$:
...
$\cN$ is a full and faithful functor from $\ABC^\forw$ to $\SBC^\forw$, from $\ABC^\back$ to $\SBC^\back$ and (with the appropriate, obvious mapping on arrows) from $\ABC^{\entails}$ to $\SBC^{\entails}$, which commute with the functors of \thcite{ab-categories} and \thcite{sb-categories}.
\end{theorem}
%
...
For a span-based branch $p=(s,c)$, let $(d'_s,u'_s)$ be the pushout cospan for $s=\spanof u d$. We recursively define $\cA$ on sb-conditions and sb-branches, as follows:
%
\begin{align*}
...
\end{lemma}
%
The following is the counterpart of \pcite{N preserves semantics}; in fact, as stated before, \pcite{N preserves semantics} follows from the property below, due to \lcite{N-A}):
%
