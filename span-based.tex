...
The step from span source shifters to span root shifters is completely identical to the arrow-based case; we omit the definition here. A new aspect is that the identity root shifter is complete.

\begin{proposition}
The identity root shifter $\cI_{A,B}$ is both a complete forward span root shifter and a complete backward span root shifter for any arrow $v:A\to B$. 
\end{proposition}
%
...
\begin{itemize}[topsep=\smallskipamount]
\item For the forward-shift case, due to \pref{sb-shifters preserve models} it follows that $h\sat \cF_i(c_j)$, and hence by the induction hypothesis the existence of $m_i:\cF_i(c_j)\to b_i$ implies $h\sat b_i$, contradicting the above.

\item For the backward-shift case, by the induction hypothesis the existence of $m_i:c_j\to \cB_i(b_i)$ implies $v_i;h\sat \cB_i(b_i)$, and hence due to \pref{sb-shifters preserve models} it follows that $h\sat b_i$, contradicting the above.
\end{itemize}
...
\begin{itemize}[topsep=\smallskipamount]
\item For the forward-shift case: by the induction hypothesis, the existence of the complete $m_i:\cF_i(c_j)\to b_i$ implies $h'\sat \cF_i(c_j)$. Since $\cF_i$ is complete, due to \pref{sb-shifters preserve models} it follows that $h=v_i;h'\sat c_j$, contradicting the above.
\item For the backward-shift case, since $\cB_i$ is complete, due to  \pref{sb-shifters preserve models} it follows that $h=v_i;h'\sat \cB_i(b_i)$, and hence by the induction hypothesis the existence of the complete $m_i:c_j\to \cB_i(b_i)$ implies $h\sat c_j$, contradicting the above.
\qed
\end{itemize}
\end{proof}
%
...
\item If $m$ is a [complete] backward-shift morphism from $b$ to $c$, then there are sb-conditions $b',c'$ such that $m$ is a [complete] forward-shift morphism from $b'$ to $c'$ using only direct forward root shifters, and there are complete forward-shift morphism from $b'$ to $b$ and from $c$ to $c'$.
\end{enumerate}
\end{proposition}
%
...
\begin{enumerate}[topsep=\smallskipamount]
\item For any [complete] forward root shifter $\cF$ for $v$, if $m$ is a [complete] forward-shift morphism from $\cF(b)$ to $c$, then $m$ is also a [complete] forward-shift morphism using from $\dcF v(b)$ to $c$ using only direct forward root shifters.

\item For any [complete] backward root shifter $\cB$ for $v$, if $m$ is a [complete] backward-shift morphism from $b$ to $\cB(c)$, then there are sb-conditions $b'\in\SC A, c'\in \SC B$ such that $m$ is a [complete] forward-shift morphism from $\dcF v(b')$ to $c'$ using only direct forward root shifters, and there are complete forward-shift morphisms $m^b:b'\to b$ and $m^c:c\to c'$.
\end{enumerate}
\end{lemma}
%
...
\begin{enumerate}[topsep=\smallskipamount]
\item Let $\cF$ be a forward root shifter for $v$, and $m=(o,(v_1,m_1)\ccdots(v_w,m_w)):\cF(b)\to c$ a forward-shift morphism. It follows that all $m_i$ are morphisms from $\cF_i(c_{o(i)})$ to $b_i$ for some forward shifter $\cF_i$; by the induction hypothesis therefore, also $m_i:\dcF{v_i}(c_{o(i)})\to b_i$. It only remains to be shown that, for all $1\leq i\leq w$, $(\cF_i(s^c_i),s^b_{o(i)})\in \cP_{v_i}$ implies $(\dcF{v_i}(s^c_i),s^b_{o(i)})\in \cP_{v_i}$. This is a consequence of \lcite{BF}.

If $m$ is complete, then all its root shifters are complete, and hence already direct.

\item Let $\cB$ be a forward root shifter for $v$, and $m=(o,(v_1,m_1)\ccdots(v_w,m_w)):\cF(b)\to c$ a backward-shift morphism. It follows that all $m_i$ are morphisms from $c_{o(i)}$ to $\cB_i(b_i)$ for some backward shifter $\cB_i$; by the induction hypothesis therefore (letting $j=o(i)$), $m_i:\dcF{v_i}(c_j')\to b_i'$ for some $c_j'\in \SC{P^c_j}$ and $b_i'\in \SC{P^b_i}$ such that there are a complete forward-shift morphisms $m^c_j:c'_j\to c_j$ and $m_i^b:b_i\to b'_i$.

Now let $b'=(A,(\dcF{v_1}(s^b_1),b_1)\ccdots (\dcF{v_w}(s^b_w),b_w))$ and 
\end{enumerate}
\end{proof}
...
\end{proof}
%
\noindent\emph{Proof of \pcite{sb-forward only}.} By induction on the depth of $m$.
\begin{enumerate}[topsep=\smallskipamount]
\item Let $m$ be a forward-shift morphism from $b$ to $c$
\end{enumerate}
