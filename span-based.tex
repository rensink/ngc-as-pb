...
\end{equation}
%
A forward span source shifter is called \emph{complete}\todo{other term?} if it also reflects models plus $v$; i.e., if the ``implies" in \eqref{eq:sb-forward condition} is an ``if and only if".
...
\end{description}
%
These options are depicted in \fcite{sb-span source shifters} (in a simplified construction that omits the intermediate steps of the span composition where possible). It turns out that, indeed, each of these four options gives rise to a valid shifter.
%
\begin{figure}
\centering
...
A final change with respect to the arrow-based case is that we also abstract the commutation condition in \dcite{ab-morphism} by relying on a family of \emph{transfer relations} $\cR_v$ over identically-sourced spans (where $v$ ranges over arrows in $\bC$), such that
%
