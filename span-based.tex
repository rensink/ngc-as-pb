...
\end{figure}
%
As we will see later, the arrows $a_i$ of an arrow-based condition can be reconstructed by taking pushouts over the spans $\spanof{u_i}{d_i}$.

\medskip\noindent Span-based conditions come with a modified notation of satisfaction. First we define what it means for a span $\spanof u d:R\to P$ to commute with a cospan $\cospanof g h$ with $g:R\to G, h:P\to G$:
\[ \spanof u d \commutes \cospanof g h \text{ if } u;g=d;h \enspace. \]
Now the definition of a span-based condition is entirely analogous to that of an arrow-based condition:
...
\begin{definition}[satisfaction of span-based conditions]\dlabel{sb-satisfaction}
  ...
  \begin{enumerate}
  \item $s_i \commutes \cospanof g h$;
  \item $h\nsat c_i$.
  \end{enumerate}
\end{definition}
%
...
\begin{figure}
  ...
  \caption{Pictorial representation of $g\sat c$ for a span-based condition $c$, with responsible branch $p_i=(\spanof{u_i}{d_i},c_i)$ and witness $h$}
  \flabel{sb-satisfaction}
\end{figure}

\subsection{Span-based root shifting}

To defined morphisms of span-based conditions, we start by extending the notion of source shifter to spans. Essentially, this consists of two things: defining what it means for a span to compose with an arrow, and using the (already defined) notion of commutation for spans instead of the corresponding notion for arrows. For the former, we use the natural
\[ \spanof a b;c \enspace:=\enspace \spanof a{b;c} \enspace \]
where $\spanof a b:A\to B$ and $c:B\to C$.

\begin{definition}[span source shifters]
Let $X,Y$ be objects. A \emph{span source shifter} $\cS$ from $X$ to $Y$ is a function from $X$-sourced spans to $Y$-sourced spans having the same target, say $Z$, such that for all spans $s:X\to Y$ and arrows $t:X\to Z$ in $\bC$:
%
\begin{equation}\eqlabel{span shifters are functors}
\cS(s;t) = \cS(s);t \enspace.
\end{equation}
%
Now let $v=A\to B$ be an arrow. $\cS$ is a \emph{forward span source shifter for $v$} if it  is a span source shifter from $A$ to $B$ such that for all spans $s:A\to C$ and all and all cospans $\cospanof g h:B\to C$:
%
\begin{equation}\eqlabel{forward condition}
s\commutes \cospanof{v;g}{h} \enspace\text{ implies }\enspace \cS(s) \commutes \cospanof{g}{h} \enspace.
\end{equation}
%
Instead, $\cS$ is a \emph{backward span source shifter for $v$} if it is a span source shifter from $B$ to $A$ such that for all spans $s:B\to C$ and all cospans $\cospanof g h:B\to C$:
%
\begin{equation}\eqlabel{backward condition}
\cS(s) \commutes \cospanof{v;g}{h} \enspace\text{ implies }\enspace s\commutes \cospanof{g}{h} \enspace.
\end{equation}
\end{definition}
...
\begin{definition}[span-based (complete) condition morphism]\dlabel{sb-morphism}
  ...
  \item for all $1\leq i\leq |b|$:
    ...
    \end{enumerate}
  \end{itemize}
  $m$ is called \emph{complete} if $o$ is surjective and for all $1\leq i\leq |b|$:
    \begin{enumerate*}[label=(\roman*)]
    \item the subcondition morphism $m_i$ is complete and 
	\item after constructing three pusouts as in the figure below, the induced morphism $e$ is an iso.
    \end{enumerate*}
\end{definition}
%
\begin{center}
