...
\end{figure}
%
As we will see later,\todo{Add ref to section} we can reconstruct an arrow-based condition from a span-based one.

As an example of span-based conditions, \fcite{sb-conditions} shows counterparts for the three arrow-based conditions in \fcite{ab-conditions}: both $b_1$ and $b_1'$ are equivalent to $c_1$ (which is rather non-obvious in the case of $b_1'$), whereas $b_2$ is equivalent to $c_2$ and $b_3$ to $c_3$.
...
$\cA$ is a (full and faithful) functor from $\SBC^{\entails}$ to $\ABC^{\entails}$, surjective on objects, which (together with $\cN$) establishes that the categories are equivalent.
\end{theorem}
