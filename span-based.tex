...
\end{figure}
%
As we will see later, we can reconstruct an arrow-based condition from a span-based one by taking pushouts over the spans $\spanof{u_i}{d_i}$; the arrows $a_i$ will then correspond to the image of $d_i$ in the pushout cospan.\todo{AR: Add definition or remove sentence, modify description of example accordingly \\ AC: maybe discuss both AB2SB and SB2AB together at the end of the section.} 

As an example of span-based conditions, \fcite{sb-conditions} shows counterparts for the three arrow-based conditions in \fcite{ab-conditions}: both $b_1$ and $b_1'$ are equivalent to $c_1$ (which is rather non-obvious in the case of $b_1'$), whereas $b_2$ is equivalent to $c_2$ and $b_3$ to $c_3$.\todo{If we include span-to-arrow conversion, add a sentence here.}

\begin{figure}[t]
\centering
\input{figs/ex-sb-conditions}
\caption{Span-based conditions corresponding to \fcite{ab-conditions}}
...
\begin{lemma}\llabel{sb-morphisms compose}
...
\end{lemma}
\else
As in the case of ab-conditions (\lcite{ab-morphisms compose}), identities and morphism composition have the expected properties. In fact, also complete forward-shift morphisms compose, but (surprisingly, at least according to us) backward-shift complete morphisms do not. This gives rise to a statement analogous to \thcite{ab-categories}.
\fi
...
Span-based conditions are richer in morphisms than arrow-based conditions, but not richer in semantics. In this subsection we show the existence of full and faithful, semantics-preserving functor from the categories of forward- and backward-shift arrow-based conditions to their span-based counterparts, as well as equivalence of the entailment-based categories.

First, we inductively define the ``natural'' span-based conditon $\cN(c)$ for an arbitrary arrow-based condition $c\in \AC(R),p\in \AB(R)$.
\begin{align*}
\cN: (R,p_1\ccdots p_w) & \mapsto (R,\cN(p_1)\ccdots \cN(p_w)) \\
      p=(a,c) & \mapsto (\spanof{\id_R}{a},\cN(c)) \enspace.
\end{align*}
%
It follows (by induction) that $\cN(c)\in \SC R$ for all $c\in \AC R$ and $\cN(p)\in \SB R$ for all $p\in \AB R$. In particular, an arrow-based branch $p=(a,c)$ (with root $R$) is translated to a span-based branch in which $R$ is the interface and the span consists of up-arrow $\id_R$ and down-arrow $a$. This transformation fully preserves the (satsfaction-based) semantics of conditions.

\begin{proposition}[$\cN$ preserves semantics]\plabel{N preserves semantics}
For any arrow-based condition $c\in \AC R$, $\cN(c)$ is a span-based condition in $\SC R$ such that $g\sat c$ iff $g\sat\cN(c)$ for any $g\of R\to G$. It follows that, for all $b,c\in \AC R$, $b\entails c$ if and only if $\cN(b)\entails \cN(c)$.
\end{proposition}
%
...
\end{enumerate*}
\todo{AR: adapt according to discussion} The latter is immediate. For the former, using $\hat \cS$ to denote span shifters, it is interesting to note that only $\hat B_v^\circ$ and $\hat\cF_v^x$ conserve the property that the left span-leg is the identity; so of the four elementary span shifters, only those two are suitable to support morphisms $\hat b\to \hat c$. Moreover, the required property $\hat\cS(\spanof{\id}{a})=\spanof{id}{\cS(a)}$ is indeed satisfied for both $\cS=\cB_v^\circ$ and $\cS=\cF_v^x$.
\qed
\end{proof}
%
...
\end{theorem}
%
From span-based to arrow-based conditions with forward- or backward-shift morphisms, there does not exist a functor, as the span-based categories are richer in morphisms.\todo{AR: add example and point to it} However, span-based conditions themselves are not richer in semantics than arrow-based ones; in other words, there exists a semantics-preserving mapping from span-based to arrow-based conditions.

For a span-based branch $p=(s,c)$, let $(d'_s,u'_s)$ be the pushout cospan for $s=\spanof u d$. We inductively define $\cA$ on sb-conditions and sb-branches, as follows:
%
\begin{align*}
...
\end{align*}
%
Note that this acts as the (left) inverse of the action of $\cN$ on objects, due to $d'_s=\id$ and $u'_s=a$ for $s=\spanof{\id}{a}$:
%
...
For any span-based condition $c\in \SC R$, $\cA(c)$ is an arrow-based condition in $\AC R$ such that $g\sat c$ iff $g\sat\cA(c)$. It follows that, for all $b,c\in \SC R$, $b\entails c$ if and only if $\cA(b)\entails \cA(c)$.
\end{proposition}
%
\begin{proof}[sketch]
This is shown by induction on the depth of $c$, using $g=d'_s;h$ iff $s\commutes (g,u'_s;h)$ for any model/witness pair $(g,h)$, in combination with the fact that $\dF{u'_s}$ preserves models adding $u'_s$ and reflects models erasing $u'_s$ (\pcite{sb-source shifters}).\qed
\end{proof}
%
\begin{theorem}[arrow-based to span-based functors]\thlabel{sb-to-ab functor}
$\cA$ is a (full and faithful) surjective functor from $\SBC^{\entails}$ to $\ABC^{\entails}$, which together with its inverse $\cN$ establishes that the categories are equivalent.
\end{theorem}
