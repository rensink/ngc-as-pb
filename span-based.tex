...
\todo[inline]{I think it's useful to have a notation for a span consisting of left leg $a$ and right leg $b$; I'm using $\spanof a b$ for this. Maybe this should also be used in span-based branches and conditions? The complementary notation for cospans would be $\cospanof ab$. See \textsf{macros.tex} for alternatives.}
...
\begin{definition}[span-based condition]\dlabel{sb-condition}
  ...
  \item $p\in \SB R$ if $p=(u,d,c)$ where $u: I\to R, d:I\func P$ form a span of arrows of $\bC$ and $c\in \SC P$.
  \end{itemize}
\end{definition}
%
In a branch $p=(u,d,c)$, $u$ stands for the \emph{up-arrow} and $d$ for the \emph{down-arrow}. As before, we use $|c|=w$ to denote the width of a span-based condition $c$, $R^c$ to denote its root, and $p^c_i=(u^c_i,d^c_i,c_i)$ its $i$-th branch. Finally, we use $I^c_i$ for the interface of branch $p^c_i$ and $P^c_i$ ($=R^{c_i}$) for its pattern (hence $u^c_i:I^c_i\func R^c$ and $d^c_i:I^c_i\rightarrow P^c_i$). In all these cases, we may omit the superscript $c$ if it is clear from the context. Pictorially, $c$ can be visualised as in \fcite{sb-condition}.
%
\begin{figure}
  \centering
...
\end{figure}
%
As we will see later, the arrows $a_i$ of an arrow-based condition can be reconstructed by taking pushouts over the spans $\tupof{u_i,d_i}$.

\medskip\noindent Span-based conditions come with a modified notation of satisfaction, defined as follows.
...
\medskip\noindent
To defined morphisms of span-based conditions, we start by extending the notion of source shifter to spans. Note that we do not work in the \emph{category} of spans, so arrows and their composition will continue to be in the base category $\bC$.

\begin{definition}
Let $X,Y$ be object. A \emph{span source shifter} $\cS$ from $X$ to $Y$ is a function from $X$-sourced spans to $Y$-sourced ones having the same target, say $Z$, such that for all arrows $t$ with source $Z$:
\[ \cS(\spanof a{b;t}) \]
\end{definition}
