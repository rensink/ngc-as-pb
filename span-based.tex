...
\begin{definition}[span-based condition]\dlabel{sb-condition}
  ...
  \begin{itemize}
  \item $c\in \SC R$ if $c=(R,p_1\cdots p_w)$ where $p_i\in \SB R$ for all $1\leq i\leq w$;
  \item $p\in \SB R$ if $p=(u,d,c)$ where $u: I\to R, d:I\func S$ form a span of arrows of $\bC$ and $c\in \SC S$.
  \end{itemize}
\end{definition}
%
\emph{Terminology and notation:} In a branch $p=(u,d,c)$, $u$ stands for the \emph{up-arrow} and $d$ for the \emph{down-arrow}. As before, we use $|c|=w$ to denote the width of a span-based condition $c$, $R^c$ to denote its root, and $p^c_i=(u^c_i,d^c_i,c_i)$ its $i$th branch. Finally, we use $I^c_i$ for the interface of branch $p^c_i$ and $R^c_i$ ($=R^{c_i}$) for the root of its subcondition (hence $u^c_i:I^c_i\func R^c$ and $d^c_i:I^c_i\rightarrow R^c_i$). In all these cases, we may omit the superscript $c$ if it is clear from the context. Pictorially, $c$ can be visualised as in \fcite{sb-condition}.
%
\begin{figure}
  \centering
  \input{figs/sb-condition}
  \caption{Pictorial representation of a span-based condition $c=(R,p_1\cdots p_w)$, with $p_i=(u_i,d_i,c_i)$ for all $1\leq i\leq w$}
  \flabel{sb-condition}
\end{figure}
%
...
%
\begin{definition}[span-based condition morphism]\dlabel{sb-morphism}
  Given two sb-conditions $b,c$, an \emph{sb-morphism} $m:b\to c$ is a triple $(t,o,m_1\cdots m_{|b|})$ such that
  \begin{itemize}
...
\end{definition}
%
\emph{Notation and terminology:} We write $t^m$ and $o^m$ for the components of $m$, and (for all $1\leq i\leq |b|$) $t^m_i$ ($=t_{m_i}$) for the top-level arrow of $m_i$ --- often omitting the superscript $m$ if it is clear from the context. Pictorially, $m$ can be visualised as in \fcite{sb-morphism}.
%
\begin{figure}
  \centering
  \input{figs/sb-morphism}
  \caption{Pictorial representation of a span-based condition morphism $m:b\func c$ (with $w=|b|$ and $m=(t,o,m_1\cdots m_w)$)}
  \flabel{sb-morphism}
\end{figure}
%
%\medskip\noindent
...
\begin{proof}
  Given an sb-condition $c=(R,p_1\cdots p_w)$, the identity morphism $\id_c:c\func c$ is inductively defined as $(\id_R,\id_w,\id_{c_1}\cdots \id_{c_w})$: the required commutativity properties trivially hold by choosing $k_i=\id_{I_i}$. By construction, $\id_c$ is complete.
  
Given two ab-condition morphisms $m:b\func c$ and $n:c\func e$, their composition is defined as $m;n=(t^m;t^n,o^m;o^n,(n_{o^m(1)};m_1)\cdots (n_{o^m(|b|)};m_{|b|}))$.
The well-definedness of $m;n$ can be proved in a standard way, choosing as intermediate morphisms $k_i=k^n_{o(i)};k^m_i$ for all $1\leq i\leq |b|$. If $m$ and $n$ are complete, so is $m;n$ (due to the fact that $o^m;o^n$ is surjective if both $o^m$ and $o^n$ are, and that pushouts compose).
...
\begin{figure}
  ...
  \caption{Pictorial representation of \pref{sb-morphisms preserve satisfaction}: if $m:b\rightarrow c$ is an sb-morphism, then $t;g\sat b$ implies $g\sat c$}
...
\begin{itemize}
\item For an arbitrary $c=(R,p_1\cdots p_w)\in \AC R$, let $\hat c=(R,\hat p_1\cdots \hat p_w)$; 
...
%
\begin{itemize}
\item For arbitrary $m:b\func c$ with $m=(t,o,m_1\cdots m_w)$, let $\hat m=(t,o,\hat m_1\cdots \hat m_w)$.
\end{itemize}
%
\begin{proposition}
