...
\SetEnumitemKey{defsep}{topsep=\smallskipamount, itemsep=\smallskipamount}
\usepackage{mathdots}
...
\newcommand{\isdef}{\stackrel{\smash{\raisebox{-1pt}{$\scriptscriptstyle\triangle$}}}{=}}
\newcommand{\iffdef}{\xLeftrightarrow{\smash{\raisebox{-1pt}{$\scriptscriptstyle\triangle$}}}}

\newcommand{\tagone}{(\dag)\xspace}
\newcommand{\tagtwo}{(\ddag)\xspace}
\newcommand{\tagthree}{\text{($*$)}\xspace}
\newcommand{\tagfour}{\text{($\circ$)}\xspace}
\newcommand{\tagfive}{(\$)\xspace}
...
\tikzset{pushout/.style={
   -{Straight Barb[black,length=5pt,width=10pt]},
   white
}}
...
\newcommand{\mygraph}[1]{%
  ...
  \begin{tikzpicture}[on grid,scale=.5,inner sep=2pt,baseline=(1.base)]
...
% ab-condition of a formula
%\newcommand{\abc}[1]{C({#1})}
%\newcommand{\abc}[1]{C[{#1}]}
\newcommand{\abc}[1]{\Gamma[{#1}]}
% sb-condition of a formula
\newcommand{\sbc}[1]{\Gamma_s[{#1}]}
% formula of an ab-condition
\newcommand{\tofol}[1]{\Phi[{#1}]}
% formula of an sb-condition
\newcommand{\sbtofol}[1]{\Phi_s[{#1}]}

% formula of a graph
\newcommand{\grtofol}[1]{\Psi[{#1}]}
\newcommand{\zeroCond}[1]{\ensuremath{(#1,\epsilon)}}


% One-edge graph with explicit edge identity
% #1: source node
% #2: edge
% #3: edge label
% #4: target node
\newcommand{\oneedgewithlab}[4]{\mygraph{
\node[n] (1) {$#1$}; 
\node[n] (2) [right=of 1] {$#4$};
\path[e]
  (1) edge[->] node[above] {$\scriptstyle{#2:\gl{#3}}$} (2);
}}

% Discrete graph with given set of nodes
\newcommand{\disc}[1]{\ensuremath{\langle #1 \rangle}}

% prefixing (back shifting) of condition with an arrow
\newcommand{\bshift}[2]{\ensuremath{{#1}\tuarr{#2}}}

% shifting a condition along an arrow
\newcommand{\shift}[2]{\ensuremath{{#1}\tdarr{#2}}}

% tight down arrow
\newcommand{\tdarr}[0]{{\mskip1mu\downarrow\mskip1mu}}
%\newcommand{\tdarr}[0]{\mathbin{\downarrow}}
%\newcommand{\tdarr}[0]{\downarrow}
% tight up arrow
\newcommand{\tuarr}[0]{{\mskip1mu\uparrow\mskip1mu}}

% merge
\renewcommand{\merge}{\sqcup}
% merge of two conditions with the same root
\newcommand{\mergeabc}[2]{\ensuremath{{#1} \sqcup {#2}}}

% join
\newcommand{\join}{\sqcap}
% join of two branches with the same root
\newcommand{\joinabc}[2]{\ensuremath{{#1} \join {#2}}}
