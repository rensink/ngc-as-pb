...
\NewEnviron{fullorname}[1][\lastlabel]{%
  \iffest
     Label: #1
     \global\expandafter\let\csname #1\endcsname\BODY
%    \expandafter\gdef\csname #1\endcsname{\BODY}
  \else
    \BODY
  \fi
}
...
\newcommand{\displayhere}[1]{\expandafter{\csname#1\endcsname}}
...
\newcommand{\flabel}[1]{\savelabel{\fname{#1}}}
...
\newcommand{\plabel}[1]{\savelabel{\pname{#1}}}
...
\newcommand{\thlabel}[1]{\savelabel{\thname{#1}}}
...
\newcommand{\llabel}[1]{\savelabel{\lname{#1}}}
...
\node[n] (1) {$#1$};
\path (1) edge [->,loop left] node {\gl{#2}} ();
}}
